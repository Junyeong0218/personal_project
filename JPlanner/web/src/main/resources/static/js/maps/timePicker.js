...
function showTimePicker(event) {
        ...
        let origin_hour;
        let origin_minute;
        if (String(currentTag.value).indexOf(":") > -1) {
                origin_time = String(currentTag.value).split(":");
                origin_hour = origin_time[0] * 1;
                origin_minute = origin_time[1] * 1;
        } else {
                origin_time = String(currentTag.value);
        ...
        const x = currentTag.offsetLeft;
        const y = currentTag.offsetTop + currentTag.offsetHeight + 5;
